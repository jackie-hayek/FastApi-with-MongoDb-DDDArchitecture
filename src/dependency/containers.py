...
class ApplicationContainer(containers.DeclarativeContainer):
    add_student_command = providers.Factory(
        AbstractAddStudentCommand.register(AddStudentCommand), student_repository=PersistenceContainer.student_repository)

    delete_student_command = providers.Factory(
        AbstractDeleteStudentByIdCommand.register(DeleteStudentByIdCommand), student_repository=PersistenceContainer.student_repository)

    update_student_command = providers.Factory(
        AbstractUpdateStudentCommand.register(UpdateStudentCommand), student_repository=PersistenceContainer.student_repository)

    get_all_students_query = providers.Factory(
        AbstractGetAllStudentsQuery.register(GetAllStudentsQuery), student_repository=PersistenceContainer.student_repository)

    get_student_byId_query = providers.Factory(
        AbstractGetStudentByIdQuery.register(GetStudentByIdQuery), student_repository=PersistenceContainer.student_repository)
